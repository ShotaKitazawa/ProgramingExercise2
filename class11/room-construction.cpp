...
class Woodfloor {  // a woodfloor class
 ...
 public:
  ...
  void draw() {
    ...
    // GLfloat lightPosition[] = {4.0f, 3.0f, 4.0f, 1.0f};  // light position
...
class Object {            // a wall class
 ...
 public:
  Object(GLdouble x0, GLdouble y0, GLdouble z0, GLdouble x1, GLdouble y1,
       GLdouble z1, GLdouble x2, GLdouble y2, GLdouble z2, GLdouble x3,
       GLdouble y3, GLdouble z3, GLdouble delta_x, GLdouble delta_y, GLdouble delta_z,GLdouble vx, GLdouble vy, GLdouble vz)
      : x0(x0+delta_x),
        y0(y0+delta_y),
        z0(z0+delta_z),
        x1(x1+delta_x),
...
Object walls[] = {
    Object(0, 5, 8, 0, 0, 8, 0, 0, 0, 0, 5, 0, 0, 0, 0, 1.0f, 0.0f, 0.0f),   // left wall
    Object(0, 5, 0, 0, 0, 0, 8, 0, 0, 8, 5, 0, 0, 0, 0, 0.0f, 0.0f, 1.0f),   // front wall
    Object(8, 5, 0, 8, 0, 0, 8, 0, 8, 8, 5, 8, 0, 0, 0, -1.0f, 0.0f, 0.0f),  // right wall
    Object(8, 5, 8, 8, 0, 8, 0, 0, 8, 0, 5, 8, 0, 0, 0, 0.0f, 0.0f, -1.0f),  // back wall
};
Object ceiling =
    Object(0, 5, 8, 0, 5, 0, 8, 5, 0, 8, 5, 8, 0, 0, 0, 0.0f, -1.0f, 0.0f);  // ceiling
Object door =
    Object(0, 5, 3, 0, 0, 3, 0, 0, 1, 0, 5, 1, 0.1f, 0, 0, 1.0f, 0.0f, 0.0f);   // left door
Object window =
    Object(5, 4, 0, 5, 1, 0, 7, 1, 0, 7, 4, 0, 0, 0, 0.1f, 0.0f, 0.0f, 1.0f);   // front window
...
void displayFunc() {
  ...
  glBindTexture(GL_TEXTURE_2D, img[1]);  // bind a named texture to a texturing
                                         // target, texture 1
  for (int i = 0; i < sizeof walls / sizeof(Object); i++) {
    walls[i].draw();  // update walls
  }
  glBindTexture(GL_TEXTURE_2D, img[2]);  // bind a named texture to a texturing
  ceiling.draw();
  glBindTexture(GL_TEXTURE_2D, img[3]);  // bind a named texture to a texturing
  door.draw();
  glBindTexture(GL_TEXTURE_2D, img[4]);  // bind a named texture to a texturing
  window.draw();
