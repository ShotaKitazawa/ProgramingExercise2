...
GLfloat radius = 0.12f;              // radius of the bouncing ball
GLfloat x = -100.0f;                   // ball's center (x,y) position
GLfloat y = 100.0f;
GLfloat x_max, x_min, y_max, y_min; // ball's center (x, y) bounds
GLfloat x_speed = 0.001f;            // ball's speed in x and y directions
GLfloat y_speed = 0.000f;
GLfloat g = 0.00098f;
GLfloat elasticity = 0.7f;
int     refresh_millis = 30;        // refresh period in milliseconds
GLfloat refresh_seconds = 0.03;
...
int main(int argc, char** argv) {
  ...
  glutInitWindowSize(640, 1280);      // initial window width and height
  glutInitWindowPosition(100, 100);  // initial window top-left corner (x, y)
  glutCreateWindow("Ball Free Falling Down"); // create window with given title
